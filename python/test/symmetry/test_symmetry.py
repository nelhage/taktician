...
class TestTransformMove(object):
    def test_transform_move(self):
        ...
        syms = [
            tak.symmetry.transform_move(s, move, 5) for s in tak.symmetry.SYMMETRIES
        ]
        assert set([tak.ptn.format_move(m) for m in syms]) == set(
            ["4a1>", "4a1+", "4e1+", "4e1<", "4a5-", "4a5>", "4e5-", "4e5<"]
        )
