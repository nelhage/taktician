...
func TestParseMove(t *testing.T) {
	cases := []struct {
		...
		{
			"a1>",
			tak.Move{X: 0, Y: 0, Type: tak.SlideRight, Slides: [8]byte{1}},
			"a1>",
			"1a1>1",
		},
		{
			"2a2<",
			tak.Move{X: 0, Y: 1, Type: tak.SlideLeft, Slides: [8]byte{2}},
			"2a2<",
			"2a2<2",
		},
		{
			"3a1+111",
			tak.Move{X: 0, Y: 0, Type: tak.SlideUp, Slides: [8]byte{1, 1, 1}},
			"3a1+111",
			"3a1+111",
		},
		{
			"5d4-22",
			tak.Move{X: 3, Y: 3, Type: tak.SlideDown, Slides: [8]byte{2, 2, 1}},
			"5d4-221",
			"5d4-221",
		},
	}
