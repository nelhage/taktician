...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default="data/pile/chunked/train-*.pt", help="datasource"
    )
    ...
    parser.add_argument(
        "--pe", type=str, default=None, help="positional encoding (sin, learned)"
    )
    ...
    loader = torch.utils.data.DataLoader(
        ds, batch_size=args.minibatch, pin_memory=True, num_workers=1
    )
    ...
    with profiler:
        for step_i in steps:
            ...
            print(
                f"[step={step_i:06d} t={now-start:.1f}s tokens={tokens:08d}] loss={avg_loss:2.2f} ms_per_step={1000*(now-step_start):.0f}"
            )
            if args.wandb:
                wandb.log(
                    {
