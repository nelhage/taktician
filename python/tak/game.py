...
class Position(object):
    ...
    def from_config(cls, config):
        size = config.size
        stones = StoneCounts(stones=config.flat_count, caps=config.capstone_count)
    ...
    def from_squares(cls, cfg, squares, ply):
        ...
        stones = (
            StoneCounts(
                cfg.flat_count - counts[0][0], cfg.capstone_count - counts[0][1]
            ),
            StoneCounts(
                cfg.flat_count - counts[1][0], cfg.capstone_count - counts[1][1]
            ),
        )
        return cls(
            size=cfg.size,
    ...
    def has_road(self):
        ...
        w = self._walk(left, pieces.Color.WHITE, True) or self._walk(
            top, pieces.Color.WHITE, False
        )
        b = self._walk(left, pieces.Color.BLACK, True) or self._walk(
            top, pieces.Color.BLACK, False
        )
        if w and b:
