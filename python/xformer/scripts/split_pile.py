...
def main():
    ...
    parser.add_argument(
        "--input", type=str, default="data/pile/train/*.zst", help="Input files"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="data/pile/chunked/train-",
        help="Prefix for output files",
    )
    parser.add_argument(
        "--n_chunks", type=int, default=64, help="Number of output chunks"
    )
    parser.add_argument(
        "--n_ctx", type=int, default=1024, help="Context length in bytes"
    )
    parser.add_argument(
        "--set_filter", type=str, default=None, help="regex to filter pile set"
    )
    parser.add_argument(
        "--read_bytes",
        type=int,
    ...
    temp_files = [args.output + f"{i:04d}.tmp" for i in range(args.n_chunks)]
    out_files = [args.output + f"{i:04d}.pt" for i in range(args.n_chunks)]

    writer = ChunkWriter(temp_files)

    in_texts = itertools.chain.from_iterable(
        xformer.data.pile_iterator(file) for file in files
    )
